...
> 与上面那条「已知未验证项」同性质。
>
> **2026-09-16 补充：正向已在真机上验过。** 用户用真实照片复现了原场景（中文系统语言、
> 设置停在「跟随系统」、拍英文），提示出现，**照提示改过识别语言后识别效果明显变好**。
> 这把上面那句「需要重新标定」从「不知成不成立」推进到「这一侧成立，且建议是对的」——
> 真实照片上判据没误判，也没把用户引到错误的语言上。
>
> **仍未验的是反方向**（拍中文不该提示），即 `massCeiling = 0.2` 那道门槛。
> 只验了正向等于只验了一半：正向的失败是「该说不说」，反向的失败是「对着做对了的人
> 说他可能错了」。清单里有对应两行。
...
| 识别语言提示**不该出现时不出现** | 同一台机器拍一张**中文**印刷体（≥30 字符），提示不应出现 | **未验证 / 待真机**（防误报，两个方向都要验） |

**识别语言提示：正向已验。** 用户真机回报：中文系统语言、设置停在「跟随系统」、拍英文文本，
提示如预期出现；**照提示改过识别语言后识别效果明显变好**。这一句比「提示出现了」更重要——
它同时验了两件事：判据在真实照片上成立（不再是合成图上的标定），以及它建议的那一门
**确实是对的那一门**。若建议的是垃圾（比如那个已被 `minimumSuggestionConfidence` 挡掉的
波兰语），用户照做只会更差。

反方向（拍中文不该提示）**仍未验证**。它防的是另一种坏：对着做对了的人说他可能错了——
`massCeiling = 0.2` 那道门槛就是为它设的。只验了正向等于只验了一半，故不写结论。
