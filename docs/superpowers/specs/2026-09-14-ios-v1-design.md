...
1. **分栏**：把所有行的横向区间求**并集**，并集内部的每一个空档就是一条候选竖缝，够宽的那条
   就是栏间距。用并集是因为**缩进天然不产生假栏**——居中标题、段首空两格会让个别行的区间变窄，
   但只要还有别的行覆盖那段 x，并集就是连续的。
...
（32 条，手造 `TextLine`）、端到端 `paracheck`（真图片走生产代码，两个方向各一条断言）、
真机。第二层是必须的——前两层的输入都是手造数据，**两头都绿推不出「真照片进去、成段的文字
出来」**，而中间那一段恰恰是本项目返工过两轮的地方。前两层都由 `build.sh` 当场运行、
失败即退非零（这一条是补的：此前两套断言只编译不运行，`paracheck` 也发现了失败却拦不住
构建，见 `tools/ocr-bench/README.md`）。
