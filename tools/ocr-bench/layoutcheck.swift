...
// 左右两栏的接缝用**同类字符**（都是中文标点）：破折号 `——`（U+2014）不在 `isCJK`
// 的区间表里，接缝上会被当成「中接拉丁」插一个空格——那是 `joinSeparator` 自己的
// 取舍，不是这一条要钉的东西，所以夹具避开它，别把两件事混在一条断言里。
let acrossGutter = column(["一句话从左栏底折过来，", "右栏顶上接着写下去"],
...
expect(TextLayout.paragraphs(from: acrossGutter),
       ["一句话从左栏底折过来，右栏顶上接着写下去，这就是栏式排版。右栏第二行在这里。"],
